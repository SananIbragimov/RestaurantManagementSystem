...
function Menu() {
  ...
  const { isOpen, onOpen, onClose } = useDisclosure();
    ...
    const fetchMenus = useCallback(async () => {
        const { data: { items, totalCount }} = await getAllMenus(currentPage, itemsPerPage);
        setMenus(items);
  ...
  return (
    ...
    <section style= {{width:'88%'}}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "15px 36px" }}>
        ...
        <MenuAddModal isOpen={isOpen} onOpen={onOpen} onClose={onClose} getMenus={fetchMenus} />
      </div>
      <div style={{ width: '100%', display: 'flex', gap: '5px', justifyContent: 'space-evenly', flexWrap: 'wrap' }}>
          {menus.map((menu) => (
                <MenuCard key={menu.id} name={menu.name} id={menu.id} sdate={menu.validFrom} edate={menu.validTo} price={menu.price} getMenus={fetchMenus}/>
              )
          )}
