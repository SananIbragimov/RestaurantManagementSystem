...
export const menuSchema = Yup.object().shape({
  ...
  validTo: Yup.date()
    .nullable()
    .when('validFrom', (validFrom, schema) => validFrom ? schema.min(Yup.ref('validFrom'), 'Valid To date must be greater than Valid From date.') : schema),
  validToTime: Yup.string()
    .nullable(),
});
